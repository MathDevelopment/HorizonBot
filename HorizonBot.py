...
async def changeroles(ctx, addorremove, user : discord.Member, role):
    if addorremove == 'add':
        await user.add_roles(role)
        ctx.send(f"Added **{role}** to {user.display_name}'s roles.")
    if addorremove == 'remove':
        await user.remove_roles(role)
        ctx.send(f"Removed **{role}** from {user.display_name}'s roles.")
    else:
        ctx.send('Please use the command in this form: `*changeroles [add/remove] [user] [role]`.')
